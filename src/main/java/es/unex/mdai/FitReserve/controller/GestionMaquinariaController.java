    ...
    public class GestionMaquinariaController {
        ...
        public String eliminarMaquinaria(@PathVariable Long id,
            ...
            try {
                ...
                // Recoger ids de reservas asociadas a esta maquinaria (evitar concurrent modification)
                Set<Long> reservaIds = maquinaria.getReservas().stream()
                ...
                // Eliminar las reservas relacionadas (si se desea eliminar todas las reservas que usan esta maquinaria)
                for (Long reservaId : reservaIds) {
                    try {
                        reservaServicio.eliminarReserva(reservaId);
                    } catch (Exception ex) {
                        // Si falla la eliminación de una reserva, continuar con las demás y acumular mensaje
                        redirectAttributes.addFlashAttribute("mensajeError",
                                "Error al eliminar alguna reserva asociada: " + ex.getMessage());
                    }
                }
                ...
                if (ok) {
                    ...
                    redirectAttributes.addFlashAttribute("mensajeError",
                            "No se ha podido eliminar la maquinaria (no existe o estaba en uso).");
                }
